...
// ---------- security headers (CSP moderada, sem inline scripts) ----------
...
const PLANS = [
  ...
  },
  {
    ...
    allowed_templates: ['T01', 'T03', 'T04'],
    price_annual: 0,
    features: ['Conferências, congressos, festivais', 'Projectos ilimitados', 'Até 10 utilizadores', 'Suporte prioritário'],
  },
  {
...
app.post('/api/auth/register', async (req, res) => {
  try {
    ...
    if (password.length < 8) return res.status(400).json({ error: 'palavra-passe deve ter no mínimo 8 caracteres' });
